...
# history route renders ProLock, and framing that under "Prove the drop-outs are
# real" ships a screenshot showing a paywall where it advertises a graph. A
# free-tier run produced exactly that, so this is a guard rather than a note.
#
# The harness writes raw/tier.txt. Absent, we warn rather than fail, so an older
# capture set still frames; present and wrong, we stop.
# `|| true`: under `set -e` an assignment whose command substitution fails takes
# the whole script down, so a missing marker would exit 0 having rendered
# nothing at all.
...
for set in $SETS; do
  ...
  python3 - "$set" <<'PY' | while IFS=$'\t' read -r out url w h; do
