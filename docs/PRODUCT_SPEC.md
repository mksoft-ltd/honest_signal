...
The status-bar icon is one of 18 pre-rendered vector drawables (3 themes × 6
levels). A runtime-generated bitmap would be more flexible but is not
guaranteed to survive every Android version's and OEM skin's monochrome
tinting path for small icons; eighteen tiny XML files buy that certainty.
...
| `POST_NOTIFICATIONS` | **The status-bar indicator *is* a notification** — its small icon is the live score. This is the app's headline feature, not an engagement channel. The app posts exactly one ongoing, silent (IMPORTANCE_LOW), non-dismissible-while-running notification and never sends any other kind. Declining leaves the whole in-app experience working. | Yes (runtime, Android 13+) |
